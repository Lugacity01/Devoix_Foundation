...
export const ImpactSummary: React.FC = () => {
  ...
  return (
    <section id="impact" className="py-16 sm:py-20 lg:py-24 bg-zinc-50 text-black relative overflow-hidden border-b border-zinc-200 selection:bg-black selection:text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Header */}
        <div className="pb-6 sm:pb-8 mb-8 sm:mb-12 border-b border-zinc-200">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5 sm:gap-4 mb-10 sm:mb-14">
          {impactStats.map((stat, idx) => (
            <div
              key={idx}
              className="p-4 sm:p-5 rounded-2xl bg-white border border-zinc-200 shadow-sm hover:shadow-xl hover:border-black hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group"
            >
              <div className="font-mono text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2 sm:mb-3">
                METRIC // 0{idx + 1}
              </div>

              <div className="font-display text-2xl sm:text-3xl lg:text-3xl xl:text-4xl font-black text-black group-hover:scale-105 transition-all mb-1 sm:mb-2 origin-left">
                {stat.number}
              </div>

              <div className="font-display text-xs sm:text-sm font-bold text-zinc-800 mb-1">
                {stat.label}
              </div>
        ...
        {/* Supporting Visual & Founder Pull Quote (Pure Black Box) */}
        <div className="relative rounded-2xl sm:rounded-3xl bg-black text-white border-2 border-zinc-800 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-0 shadow-2xl group">
          ...
          <div className="lg:col-span-7 p-6 sm:p-8 lg:p-10 flex flex-col justify-between relative z-10">
            <Quote className="w-8 h-8 sm:w-10 sm:h-10 text-zinc-700 mb-4 sm:mb-6 group-hover:text-zinc-500 transition-colors" />
            ...
            <div className="pt-4 sm:pt-6 border-t border-zinc-800 flex items-center gap-3.5">
              <img
                src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80"
                alt="Ebuka Prince"
                className="w-10 h-10 sm:w-11 sm:h-11 rounded-full object-cover border-2 border-white shrink-0 group-hover:scale-110 transition-transform grayscale"
              />
              <div>
          ...
          <div className="lg:col-span-5 relative min-h-[220px] sm:min-h-[280px] lg:min-h-[auto] bg-zinc-900 overflow-hidden">
            <img
            ...
            <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 p-3 sm:p-4 rounded-xl bg-black/95 border border-zinc-800 backdrop-blur-md">
