...
export const TeamSection: React.FC<TeamSectionProps> = ({ team = [] }) => {
  return (
    <section id="executives" className="py-16 sm:py-20 lg:py-24 bg-[#0A0A0A] text-white relative overflow-hidden border-b border-zinc-900 selection:bg-white selection:text-black">
      {/* Background Ambient Spotlights */}
      <div className="absolute top-1/3 left-1/3 w-[500px] sm:w-[600px] h-[350px] sm:h-[400px] bg-white/5 rounded-full blur-[140px] pointer-events-none animate-pulse-glow" />
      <div className="absolute bottom-10 right-10 w-[400px] h-[300px] bg-zinc-800/10 rounded-full blur-[120px] pointer-events-none animate-pulse-glow [animation-delay:3s]" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between pb-6 sm:pb-8 mb-8 sm:mb-12 border-b border-zinc-800">
          <div>
        ...
        {/* Executive Profile Cards Grid in Monochrome Black & White */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {team.map((member) => (
            <div
              key={member.id}
              className="relative p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-black border border-zinc-800 flex flex-col justify-between hover:border-white/50 hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 group overflow-hidden"
            >
              ...
              <div className="relative z-10">
                ...
                <div className="relative aspect-square rounded-xl sm:rounded-2xl overflow-hidden mb-3.5 sm:mb-4 bg-zinc-900 border border-zinc-800 shadow-inner">
                  <img
                    src={member.image}
                    alt={member.name}
                  ...
                  {member.isFounder && (
                    <span className="absolute top-2.5 left-2.5 px-2.5 py-1 rounded-md bg-black/95 border border-white/20 text-white text-[9px] font-mono font-bold uppercase tracking-wider shadow-md backdrop-blur-md flex items-center gap-1">
                ...
                <p className="text-[11px] font-mono font-semibold text-zinc-400 mb-2.5 uppercase tracking-wider">
                  {member.title}
                </p>
